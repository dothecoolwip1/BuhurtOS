...
export function ShowcasePublicPage(){
  ...
  return <div className="show-public">
    <section className="show-public-hero">
      ...
      <div className="show-public-actions"><Link className="show-btn light" to="/live">▶ Open live event view</Link><a className="show-btn glass" href="#schedule">View full schedule</a></div>
    </section>
    ...
    <section className="show-public-section" id="schedule"><div className="show-public-section-head"><div><span className="eyebrow">LIVE ACROSS THE VENUE</span><h2>Three fields. One live view.</h2></div><Link to="/live">Open live fights →</Link></div><div className="show-public-field-grid">{liveMatches.map(m=><article key={m.id}><div><span className="show-live-dot"></span><b>{m.field}</b><Pill tone="red">{m.clock}</Pill></div><small>{m.division} • {m.round}</small><section><strong>{m.left}</strong><span>{m.leftScore} : {m.rightScore}</span><strong>{m.right}</strong></section></article>)}</div></section>
    ...
    <footer className="show-public-footer"><div className="show-brand"><span className="show-brand-mark">B</span><span><b>BuhurtOS</b><small>Powered by live tournament data</small></span></div><p>Showcase information for the BuhurtOS spectator experience.</p></footer>
  </div>;
}
