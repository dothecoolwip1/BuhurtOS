...
export function TeamsPage(){
  return <>
    ...
    <div className="show-team-grid">{demoTeams.map(team=><Link to={`/teams/${team.id}`} className="show-team-card" key={team.id}>
      <div className="show-team-banner" style={{'--team':team.color} as React.CSSProperties}><span>{team.logoText}</span><Pill tone={team.status==='active'?'green':'amber'}>{team.status}</Pill></div>
