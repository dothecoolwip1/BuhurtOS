...
export function PublicPage() {
  ...
  const publicAnnouncements = announcements.filter(a => a.isPublic && (!a.scheduledFor || new Date(a.scheduledFor).getTime() <= Date.now()));
  return <div className="public-page">
    ...
    <div className="announcement-list">{publicAnnouncements.length === 0 ? <div className="state-card">No public announcements right now.</div> : publicAnnouncements.map(a => <article key={a.id}><b>{a.title}</b><p>{a.body}</p></article>)}</div>
  </div>;
}
